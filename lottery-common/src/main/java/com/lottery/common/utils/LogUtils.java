...
public class LogUtils {
     ...
     * templates
     */
    private static final String requestLogTemplate = "RequestLog: session->[{}];ip->[{}];url->[{}];method->[{}];params->[{}];";

    private static final String resultLogTemplate = "ResultLog: session->[{}];ip->[{}];url->[{}];method->[{}];timecost->[{}];results->[{}];";
    ...
    /**
     * 仅支持 现有的 php接口的返回结果，如果有再加吧。 tianjiaqin 20190710
     *
     * @return
     */
    public static String getResultFormat() {
        String resultFormat = getRequest().getParameter("result_format");
        if (StringUtils.isBlank(resultFormat)) {
            resultFormat = getRequest().getParameter("resultFormat");
        }
        return resultFormat;
    }
    ...
    public static void requestLog(Object params) {
        try {
            ...
            log.info(requestLogTemplate, request.getSession().getId(), IpUtils.getIpAddress(request), request.getRequestURL().toString(), method, paramsStr);
        } catch (Exception e) {
    ...
    public static void resultLog(Object results) {
        try {
            ...
            log.info(resultLogTemplate, request.getSession().getId(), IpUtils.getIpAddress(request), request.getRequestURL().toString(), method, getTimeCost(), JSONObject.toJSONString(results));
        } catch (Exception e) {
            exceptionLog(e.getMessage());
        }
    }
